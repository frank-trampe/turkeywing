...
function formatter(req, res, next) {
  <span class="missing-if-branch" title="if path not taken" >I</span>if(res.data === undefined) {
...
&nbsp;
    ...
    app.providers.push(function (path, service, options) {
      const uri = p</span>ath.indexOf('/') === 0 ? <span class="branch-0 cbranch-no" title="branch not covered" >path : `/${path}`;
      const baseRoute = app.route(uri);
      const idRoute = app.route(`${uri}/:__feathersId`);
&nbsp;
      l</span>et middleware = (</span>options || <span class="branch-1 cbranch-no" title="branch not covered" >{}).middleware || <span class="branch-1 cbranch-no" title="branch not covered" >{};
      l</span>et before = middleware.before || <span class="branch-1 cbranch-no" title="branch not covered" >[];
      l</span>et after = middleware.after || <span class="branch-1 cbranch-no" title="branch not covered" >[];
&nbsp;
      <span class="missing-if-branch" title="else path not taken" >E</span>if(typeof handler === 'function') {
